...
const input = { name: ' Camille ', email: ' camille@example.com ', phone: ' +33 6 00 00 00 00 ', message: ' Entretenir mes haies. ' };
...
function fixture({ dbFailure = false, emailStatus = 200, emailFailure = false, configured = true } = {}) {
  ...
  const notification = loadModule('lib/contact-notification.ts', {}, {
    process,
...
test('save precedes notification, preserves Prisma fields and sets reply_to', async () => {
  ...
  assert.deepEqual(result.contact, { id: 42, createdAt: '2026-09-16T00:00:00.000Z', name: 'Camille', email: 'camille@example.com', phone: '+33 6 00 00 00 00', message: 'Entretenir mes haies.' });
...
test('optional phone is stored as null when absent, blank or null', async () => {
  for (const phone of [undefined, '', '   ', null]) {
    const f = fixture();
    const result = await (await f.post({ ...input, phone })).json();
...
test('all client-provided HTML is escaped and plaintext retains the message', async () => {
  ...
  await f.post({ name: '<b>A & B</b>', email: "o'connor@example.com", phone: '<i>123</i>', message });
  ...
  assert.ok(html.includes('o&#39;connor@example.com'));assert.ok(html.includes('&lt;i&gt;123&lt;/i&gt;'));
...
for (const [label, options] of [
  ...
  test(`${label}: contact stays saved and response is explicitly partial success`, async () => {
    ...
    assert.equal(f.saved.length, 1);assert.equal(result.contact.id, 42);
